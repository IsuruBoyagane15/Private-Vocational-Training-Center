<?php
  ...
  function getAvailableID($module_name, $assign_name, $deadline="0000-00-00 00:00:00", $assign_desc, $late_allowed, $attempts) {
    ...
    if($result_set) {
      while( $record = mysqli_fetch_assoc($result_set) ) {
        if( $record['is_deleted'] == 1 ) {
          ...
          $query = "UPDATE config_createassignment SET tableName='assignment{$availableId}', module='{$module_name}', assignment_name='{$assign_name}', description='{$assign_desc}', late_allowed='{$late_allowed}', deadline='{$deadline}', no_of_attempts='{$attempts}', is_deleted=0 WHERE id={$availableId}";
    ...
    $query = "INSERT INTO config_createassignment (id, tableName, module, assignment_name, deadline, is_deleted, description, late_allowed, no_of_attempts) VALUES ('{$availableId}', 'assignment{$availableId}', '{$module_name}', '{$assign_name}', '{$deadline}', 0, '{$assign_desc}', '{$late_allowed}', '{$attempts}')";
    $result = mysqli_query($conn, $query);
    if(!$result) {
      echo "configdata database query failed! " . mysqli_error($conn) . "<br>" ;
    }
  ...
  function createTable($conn, $module_name, $assign_name, $deadline="NULL", $assign_desc, $late_allowed, $attempts) {
    $avaiId = getAvailableID($module_name, $assign_name, $deadline, $assign_desc, $late_allowed, $attempts);
